...
const Scene = loom.Scene;

const FnType = ?(*const fn (self: *anyopaque, scene: *Scene) anyerror!void);
...
pub fn callSafe(self: *Self, event: Events, scene: *Scene) void {
...
inline fn determineFunctionType(comptime T: type, comptime info: std.builtin.Type.Fn) ?FunctionType {
    switch (info.params.len) {
        2 => {
            if (info.params[0].type == *T and info.params[1].type == *Scene) return FunctionType.generic;
            if (info.params[0].type == *Scene and info.params[1].type == *T) return FunctionType.reversed;
        },
        1 => {
            if (info.params[0].type == *T) return FunctionType.self_only;
            if (info.params[0].type == *Scene) return FunctionType.scene_only;
        },
        0 => {
...
fn attachEvents(self: *Self, comptime T: type) void {
    ...
    const wrapper = struct {
        fn call(comptime fn_name: []const u8, cache: *anyopaque, scene: *Scene) !void {
            ...
            if (comptime (typeinfo.return_type.? == void))
                switch (comptime determineFunctionType(T, typeinfo) orelse return) {
                    .generic => @call(.auto, func, .{ @as(*T, @ptrCast(@alignCast(cache))), scene }),
                    .reversed => @call(.auto, func, .{ scene, @as(*T, @ptrCast(@alignCast(cache))) }),
                    .self_only => @call(.auto, func, .{@as(*T, @ptrCast(@alignCast(cache)))}),
                    .scene_only => @call(.auto, func, .{scene}),
        ...
        pub fn awake(cache: *anyopaque, scene: *Scene) !void {
            try call("Awake", cache, scene);
        }

        pub fn start(cache: *anyopaque, scene: *Scene) !void {
            try call("Start", cache, scene);
        }
        pub fn end(cache: *anyopaque, scene: *Scene) !void {
            try call("End", cache, scene);
        }

        pub fn update(cache: *anyopaque, scene: *Scene) !void {
            try call("Update", cache, scene);
        }
        pub fn tick(cache: *anyopaque, scene: *Scene) !void {
            try call("Tick", cache, scene);
        }
    };
